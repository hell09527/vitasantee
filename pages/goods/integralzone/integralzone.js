...
Page({
  ...
  onLoad: function (options) {
    ...
    app.sendRequest({
      url: 'api.php?s=goods/getIntegralCenterGoods',
      data: {},
      success: function (res) {
        ...
        console.log(res);
        if (code == 0) {
          let integral_adv = data.integral_adv;
          let goods_list = data.goods_list;

          for (let index in integral_adv.adv_list) {
            let img = integral_adv.adv_list[index].adv_image;
            integral_adv.adv_list[index].adv_image = app.IMG(img);
          }
