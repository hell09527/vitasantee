...
Page({
  ...
  formatGoods: function (goods_list) {
    "use strict";
    for (let i = 0, flag = true, len = goods_list.length; i < len; flag ? i++ : i) {
      if (goods_list[i].category_id == 0 || goods_list[i].category == null || goods_list[i].goods_info.picture_info == null || goods_list[i].goods_info.picture_info == undefined) {
  ...
  onLoad: function (options) {
    ...
    app.sendRequest({
      ...
      success: function (res) {
        ...
        if (code == 0) {
          let goods_list = that.formatGoods(data.data);
          ...
          for (let index in goods_list) {
            ...
            if (img == '') {
              goods_list[index].goods_info.picture_info.pic_cover_small = '0';
            }
          }
          that.setData({
